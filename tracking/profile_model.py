...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='rsfnet', choices=['rsfnet'],
...
def evaluate_vit(model, search, temp, temp1,temp2,search_i,temp_i,temp1_i, temp2_i,):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(search, temp, temp1,temp2,search_i,temp_i,temp1_i, temp2_i,),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(search, temp, temp1,temp2,search_i,temp_i,temp1_i, temp2_i)
        start = time.time()
        for i in range(T_t):
            _ = model(search, temp, temp1,temp2,search_i,temp_i,temp1_i, temp2_i)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "rsfnet":
        ...
        if merge_layer <= 0:
            evaluate_vit(model, search,template,template, template,search,template,template, template)
        else:
