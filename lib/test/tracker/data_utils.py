...
class Preprocessor(object):
    ...
    def process(self, img_arr: np.ndarray,img_arr_i, amask_arr: np.ndarray):
        # Deal with the image patch
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        img_tensor_i = torch.tensor(img_arr_i).cuda().float().permute((2,0,1)).unsqueeze(dim=0)

        img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W)
        img_tensor_norm_i = ((img_tensor_i / 255.0) - self.mean) / self.std  # (1,3,H,W)

        # Deal with the attention mask
        amask_tensor = torch.from_numpy(amask_arr).to(torch.bool).cuda().unsqueeze(dim=0)  # (1,H,W)


        return NestedTensor(img_tensor_norm, amask_tensor),NestedTensor(img_tensor_norm_i, amask_tensor)
