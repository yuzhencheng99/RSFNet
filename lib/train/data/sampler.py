...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        self.samples_per_epoch = samples_per_epoch      #1000
        self.max_gap = max_gap                  #100     搜索和模板之间最大间隔帧数
        self.num_search_frames = num_search_frames       #batch size
        self.num_template_frames = num_template_frames     #batch size
        self.processing = processing      #processing中定义的预处理套路
        self.frame_sample_mode = frame_sample_mode          #'interval'
    ...
    def __getitem__(self, index):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'interval':
                    ...
                    while search_frame_ids is None:
                        ...
                        online_frame_ids =self._sample_visible_ids(visible, num_ids=1)
                        online1_frame_ids =self._sample_visible_ids(visible, num_ids=1)
                    ...
                    while search_frame_ids is None:
                        ...
                        online_frame_ids = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                                 max_id=len(visible) - self.num_search_frames)
                        online1_frame_ids = self._sample_visible_ids(visible, num_ids=1,
            ...
            try:
                ...
                template_frames, template_anno, meta_obj_template = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict, frame_type='visible')
                if template_frames == None:
                    print(seq_id)
                online_frames, online_anno, meta_obj_online = dataset.get_frames(seq_id, online_frame_ids, seq_info_dict, frame_type='visible')
                online1_frames, online1_anno, meta_obj_online1 = dataset.get_frames(seq_id, online1_frame_ids, seq_info_dict, frame_type='visible')
                search_frames, search_anno, meta_obj_search = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict, frame_type='visible')

                template_frames_i, _,_ = dataset.get_frames(seq_id, template_frame_ids,seq_info_dict, frame_type='infrared')
                online_frames_i, _,_ = dataset.get_frames(seq_id, online_frame_ids,seq_info_dict, frame_type='infrared')
                online1_frames_i, _,_ = dataset.get_frames(seq_id, online1_frame_ids,seq_info_dict, frame_type='infrared')

                search_frames_i, _,_ = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict, frame_type='infrared')
                # x=torch.sum(template_anno['bbox'][0])
                # if template_anno['bbox'][0][-1]!=0 and template_anno['bbox'][0][-2]!=0 and search_anno['bbox'][0][-1]!=0 and search_anno['bbox'][0][-2]!=0 and online_anno['bbox'][0][-1]!=0 and online_anno['bbox'][0][-2]!=0 and online1_anno['bbox'][0][-1]!=0 and online1_anno['bbox'][0][-2]!=0:
                #     break

                H,W,_=search_frames[0].shape
                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H, W))]
                template_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H, W))] *3
                # print(dataset.name)
                # print(dataset.sequence_list[seq_id])
                data = TensorDict({'template_images': template_frames,
                                   'template_images_i': template_frames_i,
                                   'template_anno': template_anno['bbox'],
                                   'template_masks': template_masks,
                                   'online_images': online_frames,
                                   'online_images_i': online_frames_i,
                                   'online_anno': online_anno['bbox'],
                                   'online_masks': template_masks,
                                   'online1_images': online1_frames,
                                   'online1_images_i': online1_frames_i,
                                   'online1_anno': online1_anno['bbox'],
                                   'online1_masks': template_masks,
                                   'search_masks': search_masks,
                                   'search_images': search_frames,
                                   'search_images_i': search_frames_i,
