...
class BaseProcessing:
    ...
    def __init__(self, transform=transforms.ToTensor(), template_transform=None, search_transform=None, joint_transform=None):
        ...
        self.transform = {'template': transform if template_transform is None else template_transform,
                          'online': transform if template_transform is None else template_transform,
                          'online1': transform if template_transform is None else template_transform,
...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, template_area_factor,search_sz,temp_sz, center_jitter_factor, scale_jitter_factor,
        ...
        self.search_area_factor = {'search':search_area_factor,'template':template_area_factor,'online':template_area_factor,'online1':template_area_factor}
        self.output_sz = {'search':search_sz,'template':temp_sz,'online':temp_sz,'online1':temp_sz}
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['online_images'], data['online_anno'], data['online_masks'] = self.transform['joint'](
                image=data['online_images'], bbox=data['online_anno'], mask=data['online_masks'])
            data['online1_images'], data['online1_anno'], data['online1_masks'] = self.transform['joint'](
                image=data['online1_images'], bbox=data['online1_anno'], mask=data['template_masks'])
            data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)

            data['template_images_i'],_,_ = self.transform['joint'](
                image=data['template_images_i'], bbox=data['template_anno'], mask=data['template_masks'])
            data['online_images_i'], _,_ = self.transform['joint'](
                image=data['online_images_i'], bbox=data['online_anno'], mask=data['online_masks'])
            data['online1_images_i'], _,_= self.transform['joint'](
                image=data['online1_images_i'], bbox=data['online1_anno'], mask=data['template_masks'])
            data['search_images_i'], _,_ = self.transform['joint'](
                image=data['search_images_i'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)

        for s in ['template', 'search','online','online1']:
            ...
            crops_i, boxes_i, att_mask_i, mask_crops_i = prutils.jittered_center_crop(data[s + '_images_i'], jittered_anno,
                                                                              data[s + '_anno'],
            ...
            data[s + '_images_i'],_,_,_ = self.transform[s](
                image=crops_i, bbox=boxes_i, att=att_mask_i, mask=mask_crops_i, joint=False)
