...
def candidate_elimination(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    ...
    # aaa=attn.detach().cpu().numpy()[0,0,:,:]
    if box_mask_z is not None:
        box_mask_z = box_mask_z.unsqueeze(1).unsqueeze(-1).expand(-1, attn_t.shape[1], -1, attn_t.shape[-1])
        # attn_t = attn_t[:, :, box_mask_z, :]
        # x=torch.count_nonzero(box_mask_z[:,0,:,:]==True) # each search token has a similarity value to the center template token
        attn_t = attn_t[box_mask_z]
        attn_t = attn_t.view(bs, hn, -1, lens_s)
        attn_t = attn_t.mean(dim=2).mean(dim=1)  # B, H, L-T, L_s --> B, L_s  average over n heads
    ...
    #*********************
    a = int(lens_t + lens_s / 2)
    attn_t1 = attn[:, :, a:, lens_t:a]  # bz hn lent lens

    attn_t2 = attn[:, :, lens_t:a, a:]
    t1s = attn_t1.shape[2]
    t2s = attn_t2.shape[2]
    if t1s < t2s:
        attn_t1 = attn[:, :, a - 1:, lens_t:a]  # bz hn lent lens
    if t1s > t2s:
        attn_t2 = attn[:, :, lens_t - 1:a, a:]

    attn_tp = torch.concat((attn_t1, attn_t2), dim=-1)
    attn_tp = attn_tp.mean(dim=2).mean(dim=1)  # B, H, L-T, L_s --> B, L_s
    attn_t = attn_t + attn_tp
    #***********************

    # use sort instead of topk, due to the speed issue
    # https://github.com/pytorch/pytorch/issues/22812
    # attn_t (bz lens) the attention weight of each search token to the center temp token
    sorted_attn, indices = torch.sort(attn_t, dim=1, descending=True)  # sort search tokens by attention weight

    topk_attn, topk_idx = sorted_attn[:, :lens_keep], indices[:, :lens_keep]
    non_topk_attn, non_topk_idx = sorted_attn[:, lens_keep:], indices[:, lens_keep:]

    keep_index = global_index.gather(dim=1, index=topk_idx)  #keep topk_index tokens
    removed_index = global_index.gather(dim=1, index=non_topk_idx)  # 1- topk
